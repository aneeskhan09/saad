...
class Admin extends CI_Controller
{
    ...
    /****** View Orders *******/
    function orders_managment()
    {
        if($this->isLoggedIn())
        {
            if($_POST):
                extract($_POST); //print_r($_POST);die;
                $this->Admin_model->update_order_status(array('status' => $_POST['status']),array('id'=>$_POST['order_id']));
            ...
            $data['menu'] = $this->Admin_model->getMenuItems('admin_menu');
            $data['theme'] = $this->Admin_model->getActiveTheme();
            $data['ordersdata'] = $this->Admin_model->getall_order($this->session->userdata('id'));
            $data['company_info'] = $this->Admin_model->get_company_info();
            $data['title'] = $data['company_info']['name']." | Order Managment";
            $this->load->view('backend/static/head', $data);
            $this->load->view('backend/static/header');
            $this->load->view('backend/static/sidebar1');
            $this->load->view('backend/orders_managment',$data);
            $this->load->view('backend/static/footer');
        }
        else
        {
